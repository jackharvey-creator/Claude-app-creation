...
export interface EvaluationInput {
  firstName: string; lastName: string; answers: AssessmentAnswer[]
  rawScore: number; maxScore: number; percentScore: number; passed: boolean
  resumeText?: string
  linkedInData?: { headline?: string; summary?: string; positions?: Array<{ title: string; company: string; duration: string }>; skills?: string[] }
}

export interface EvaluationResult { summary: string; strengths: string[]; concerns: string[]; recommendation: string }
...
export async function evaluateCandidate(input: EvaluationInput): Promise<EvaluationResult> {
  ...
  const stream = client.messages.stream({
    ...
    system: 'You are a recruitment expert for Comparion Insurance. Evaluate candidates fairly and concisely. Always respond with valid JSON only — no markdown, no extra text.',
  })
  ...
  try {
    ...
    return { summary: parsed.summary ?? '', strengths: Array.isArray(parsed.strengths) ? parsed.strengths : [], concerns: Array.isArray(parsed.concerns) ? parsed.concerns : [], recommendation: parsed.recommendation ?? '' }
  } catch {
    return { summary: `${input.firstName} scored ${input.percentScore}% on the readiness assessment.`, strengths: ['Completed the full assessment'], concerns: ['Unable to generate detailed AI analysis'], recommendation: input.passed ? 'Recommend reviewing profile and scheduling an interview.' : 'Score below threshold — review manually before proceeding.' }
  }
}
