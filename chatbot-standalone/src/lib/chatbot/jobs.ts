export interface JobLocation { id: string; title: string; office: string; city: string; state: string; stateCode: string; lat: number; lng: number; url: string }
...
function degreesToRadians(d: number) { return d * (Math.PI / 180) }
...
export interface NearestJobResult { job: JobLocation; distanceMiles: number }

export function findNearestJob(lat: number, lng: number): NearestJobResult {
  let nearest = JOB_LOCATIONS[0]; let minDist = Infinity
  for (const job of JOB_LOCATIONS) { const dist = haversineDistanceMiles(lat, lng, job.lat, job.lng); if (dist < minDist) { minDist = dist; nearest = job } }
  return { job: nearest, distanceMiles: Math.round(minDist) }
}

export async function resolveLocationFromIp(ip: string): Promise<{ city: string; state: string; lat: number; lng: number } | null> {
  try {
    const cleanIp = ip.replace(/^::ffff:/, '')
    const res = await fetch(`https://ipapi.co/${cleanIp}/json/`, { headers: { 'User-Agent': 'ComparionRecruitmentBot/1.0' } })
    if (!res.ok) return null
    const data = await res.json() as { city?: string; region?: string; latitude?: number; longitude?: number; error?: boolean }
    if (data.error || !data.latitude || !data.longitude) return null
    return { city: data.city ?? '', state: data.region ?? '', lat: data.latitude, lng: data.longitude }
  } catch { return null }
}
