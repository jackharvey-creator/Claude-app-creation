    ...
    <div className="min-h-screen bg-gradient-to-br from-blue-50 via-white to-indigo-50 flex flex-col">
      ...
      <main className="flex-1 flex items-start justify-center px-4 py-8">
        <div className="w-full max-w-lg">
          {step.type === 'questions' && (
            <div className="mb-6">
              <ProgressBar current={step.index + 1} total={QUESTIONS.length} label={`Question ${step.index + 1} of ${QUESTIONS.length}`} />
            </div>
          )}
          ...
          <div className="bg-white rounded-2xl shadow-sm border border-gray-100 p-6 sm:p-8">
            ...
            {step.type === 'questions' && currentQ && (
              <QuestionStep key={currentQ.id} question={currentQ} onAnswer={handleAnswer} selectedAnswerId={answers.find((a) => a.questionId === currentQ.id)?.answerId} />
            )}
