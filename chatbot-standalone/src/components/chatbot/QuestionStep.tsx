        ...
        {question.options.map((option) => {
          ...
          return (
            <button key={option.id} onClick={() => handleSelect(option.id, option.points)}
              ...
              <span className={`inline-flex items-center justify-center w-7 h-7 rounded-full text-xs font-bold mr-3 ${isSelected ? 'bg-blue-600 text-white' : 'bg-gray-100 text-gray-500'}`}>{option.id.toUpperCase()}</span>
              {option.text}
            </button>
          )
        })}
