...
interface AssessRequest {
  ...
  // Location (from browser geolocation or manual entry)
  latitude?: number
  longitude?: number
  city?: string
  state?: string
...
export async function POST(req: NextRequest) {
  try {
    ...
    // Resolve location — use provided coords or fall back to IP geolocation
    ...
    // Find nearest Comparion office
    const nearest = lat && lng ? findNearestJob(lat, lng) : null

    // Run Claude AI evaluation
    ...
    // Send notifications (non-blocking — don't fail the request if emails fail)
    ...
    // Update invite-sent timestamp if applicable
    if (score.passed) {
