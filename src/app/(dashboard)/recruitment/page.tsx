...
export const metadata: Metadata = {
  title: 'Recruitment | Comparion Insurance',
}
...
function ScoreBadge({ pct, passed }: { pct: number; passed: boolean }) {
  return (
    <span className={`inline-block rounded-full px-2 py-0.5 text-xs font-bold ${passed ? 'bg-green-100 text-green-700' : 'bg-red-100 text-red-700'}`}>
      {pct}%
    </span>
  )
}
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        ...
        <div className="bg-white rounded-xl border border-gray-200 p-5 mb-6">
          ...
          <p className="text-xs text-gray-400 mt-3">
            Append <code>?source=</code> to track where candidates are coming from. All links open to the same assessment.
          </p>
        </div>
              ...
              <table className="w-full text-sm">
                <thead>
                  <tr className="border-b border-gray-100 bg-gray-50">
                    ...
                    <th className="text-left px-4 py-3 text-xs font-semibold text-gray-500 uppercase tracking-wide">Nearest Office</th>
                  ...
                  {assessments.map((a: CandidateAssessment) => {
                    ...
                    return (
                      <tr key={a.id} className="border-b border-gray-50 hover:bg-gray-50 transition-colors">
                        ...
                        <td className="px-4 py-3.5">
                          <span className={`inline-block rounded-full px-2 py-0.5 text-xs font-medium ${STATUS_COLORS[a.status] ?? 'bg-gray-100 text-gray-600'}`}>
                            {a.status}
                          </span>
                        </td>
